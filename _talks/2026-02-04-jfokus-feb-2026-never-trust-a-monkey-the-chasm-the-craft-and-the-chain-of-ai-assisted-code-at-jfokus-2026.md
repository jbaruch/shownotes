---
layout: talk
---

# Never Trust a Monkey: The Chasm, the Craft, and the Chain of AI-Assisted Code at JFokus 2026

**Conference:** JFokus 2026
**Date:** 2026-02-04
**Slides:** [View Slides](https://drive.google.com/file/d/1cbwtwcwZpAUhcc1XhDg9h0qUEZE6Tkci/preview)
**Video:** [View Video](https://youtu.be/P34QBbvLXjU?si=0Q62xa2txO9uRGfo)

## Abstract

Translating ideas into code has always been messy. Specs, user stories, and endless discussions often leave developers guessing: "Is this what they meant?" Misunderstandings pile up, and the real work—writing great code—gets bogged down in back-and-forths. What if there was a way to smooth that process? Imagine starting with a prompt, written in plain language, that describes what a feature should do. From there, the AI generates tests, turning the prompt into something concrete—something the whole team can align on. And when those tests look right, the AI writes the code to make them pass. It's not about skipping steps but cutting through the noise, so developers can focus on solving problems and shipping great features. Could this be how we close the gap between product intent and implementation? Could test-first workflows like this make quality implicit and productivity effortless? In this talk, we'll explore these possibilities, from the way prompts reshape collaboration to the role of tests in bridging understanding—and how AI fits into it all. Maybe building software doesn't need to feel like a grind. What if it could feel… easier?

## Resources

- [Intent-Integrity-Kit - Framwork that implments the Intent Integrity Chain](https://github.com/intent-integrity-chain/kit)
- [Evaluating the Code Quality of AI-Assisted Code Generation Tools](https://arxiv.org/abs/2304.10778)
- [Assessing the Quality and Security of AI-Generated Code: A Quantitative Analysis](https://arxiv.org/html/2508.14727v1)
- [Qodo Report: 2025 Stateof AI code quality](https://www.qodo.ai/reports/state-of-ai-code-quality/)
- [StackOverflow Developer Survey 2025 – AI Tools (Dis)trust](https://survey.stackoverflow.co/2025/ai#2-accuracy-of-ai-tools)
- [Faster code, greater risks: The security trade-off of AI-driven development](https://apiiro.com/blog/faster-code-greater-risks-the-security-trade-off-of-ai-driven-development/)
- [Baruch’s Book The Liquid Software](https://amzn.to/4jXSS3X)
- [Baruch’s Book DevOps Tools for Java Developers](https://amzn.to/4mjQje4)
- [Aider's Code editing leaderboard](https://aider.chat/docs/leaderboards/edit.html)
- [Curse of knowledge](https://en.wikipedia.org/wiki/Curse_of_knowledge)
- [Stochastic](https://en.wikipedia.org/wiki/Stochastic)
- [Martin's Fowler's Understanding Spec-Driven-Development: Kiro, spec-kit, and Tessl](https://martinfowler.com/articles/exploring-gen-ai/sdd-3-tools.html)
- [ThoughtWorks's Spec-driven development - Unpacking one of 2025’s key new AI-assisted engineering practices](https://www.thoughtworks.com/en-us/insights/blog/agile-engineering-practices/spec-driven-development-unpacking-2025-new-engineering-practices)
- [GitHub's Spec-kit](https://github.com/github/spec-kit)
- [OpenSpec](https://github.com/Fission-AI/OpenSpec/)
- [Amazon’s Kiro – Spec-Driven AI IDE](https://kiro.dev/)
- [Zencoder - AI platform with spec-driven workflows](https://docs.zencoder.ai/user-guides/tutorials/spec-driven-development-guide#a-practical-guide-to-spec-driven-development)
- [Andrej Karpathy on SSD](https://x.com/karpathy/status/2015887154132746653)
- [Arun Gupta's talk on SSD at JFokus 2026](https://www.jfokus.se/jfokus26-preso/Spec-driven-Development-using-Coding-Agents.pdf)
- [Test Driven Development](https://en.wikipedia.org/wiki/Test-driven_development)
- [Martin Fowler's Given When Then](https://martinfowler.com/bliki/GivenWhenThen.html)
- [Gherkin Specs](https://cucumber.io/docs/gherkin/)
- [Behavior Driven Development](https://en.wikipedia.org/wiki/Behavior-driven_development)
- [Tessl.io – Make agents work in real codebases](https://tessl.io/)