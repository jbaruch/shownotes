...
Baruch brings Claude Code on Opus 4.7. Viktor brings JetBrains' **Junie** running Gemini Flash 3.5. Same prompts, same hardware (a smart bulb, a camera, two LED light bars), different agents. Five stages, escalating complexity — and the demo opens with the most damning beat of all: you're in a Kotlin Gradle project, you ask for "a program that turns on my smart bulb"… and the agent emits **Python**. One `tessl install jbaruch/kotlin-tutor` later, same prompt, the same agent emits Kotlin with Ktor, Gradle Kotlin DSL, and the right JDK toolchain. Different file extension, different stack, identical prompt. Context engineering before we've even started the camera.

Stage 1–2: face detection + recognition wired up in Kotlin via JavaCV + DJL. Both agents succeed.

Stage 3 — the first quiet aha: drive a confidence semaphore on Govee H6056 light bars. Both agents fail silently. The API returns 200 OK on commands targeting segments that physically don't exist. The textbook confidence formula compresses strong matches into the middle band. The fill direction is upside down. Four bugs, zero exceptions. Four Tessl plugins encoding device ground truth + calibration + actuator patterns + vision-pipeline foundations fix all of it in one `tessl install` beat — same agent, same prompt, four bugs gone.

Stage 4 — the second aha: decompose into sub-agents using **Koog** (JetBrains' Kotlin-native agent framework). The orchestrator dispatches to three sub-agents (vision, IoT, evaluation). All four plugins from Stage 3 stay loaded on the parent — and none of them reach the sub-agents. Each sub-agent reinvents reality: textbook formulas, phantom segment addresses, no stability filter, emotion bar dark. The `sub-agent-delegation` meta-plugin teaches the orchestrator the explicit-handoff pattern (system-prompt skill injection + echo validation), and the entire pipeline snaps back to correctness.

This isn't a framework comparison. It's a live demonstration that for Kotlin developers in 2026, "AI engineering" means **engineering the discipline around your agent** — the language defaults, the device facts, the calibration constants, the actuator patterns, the sub-agent handoff. Six plugins, an `tessl install`, and a stopwatch. The headline number is **27% → 100%** — measurable, reproducible, on the registry.
