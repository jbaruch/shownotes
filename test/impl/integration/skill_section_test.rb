...
#
# FixtureSiteSkillSectionTest builds the real layouts, homepage, and assets
# with isolated demo fixtures, keeping sample content out of the live site.
#
...
class FixtureSiteSkillSectionTest < Minitest::Test
  include SkillSectionHelpers

  def self.site
    @site ||= begin
      source = Dir.mktmpdir('skill_fixture_site')
      Minitest.after_run { FileUtils.rm_rf(source) }
      %w[_layouts _includes assets index.md _config.yml].each do |path|
        FileUtils.cp_r(File.join(REPO_ROOT, path), source)
      end
      %w[_talks _skills].each do |path|
        FileUtils.cp_r(File.join(REPO_ROOT, 'test', 'fixtures', 'talk_skills', path), source)
      end
  ...
  def expected_name
    skill_front_matter(File.join(site.source, '_skills', SKILL_STEM, 'SKILL.md'))['name']
  end

  def expected_description
    skill_front_matter(File.join(site.source, '_skills', SKILL_STEM, 'SKILL.md'))['description']
  end
  ...
  def test_copy_script_is_served_and_referenced_only_on_skill_pages
    ...
    assert File.exist?(File.join(site.dest, 'assets', 'js', 'skill-install.js')),
  ...
  def test_homepage_cards_show_the_badge_only_for_the_skill_talk
    home = Nokogiri::HTML(File.read(File.join(site.dest, 'index.html')))
  ...
  def test_raw_file_is_served_byte_identical
    source = File.binread(File.join(site.source, '_skills', SKILL_STEM, 'SKILL.md'))
    served = File.join(site.dest, 'skills', SKILL_STEM, 'SKILL.md')
